...
/*   Updated: 2025/05/18 01:22:54 by michoi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_single_cmd(const char **builtins, t_cmd *cmd, t_env *env)
{
	...
	else
	{
		...
		if (waitpid(child_pid, &wait_stat, 0) == -1)
			return (FAILURE);
		// if (cmd_args->infile_fd > -1)
		// 	close_fd(cmd_args->infile_fd);
	}
	// !!!!exit stat code!!!!
	return (SUCCESS);
}
...
{
	const char	*builtins[] = {CMD_CD, CMD_ECHO, CMD_ENV, CMD_EXPORT, CMD_EXIT, CMD_PWD,
			CMD_UNSET, NULL};
	t_pipe		cmd_pipe;
	pid_t		child_pid;
	int			wait_stat;
